...
if __name__ == '__main__':
    ...
    # set up sim
    randAngle = (np.random.rand()-0.5)*3.14
    goalX = 5.0*np.cos(randAngle)
    goalY = 5.0*np.sin(randAngle)
    ...
    sim.newTerrain(goal=[goalX,goalY])
    sim.resetRobot()
    p.addUserDebugLine([goalX,goalY,0],[goalX,goalY,10],lineColorRGB=[0,1,0],lineWidth=2,physicsClientId=sim.physicsClientId)
